...
describe('Simple Workflow Integration Test', () => {
  ...
  describe('Sample Flow Execution', () => {
    it('should create application and test complete workflow', async () => {
      ...
      // Step 5: Transition to UNDER_SCRUTINY
      ...
      // Step 6: Create BCA review (using BCA section as OWO equivalent)
      ...
      await prisma.review.create({
        data: {
          ...
          remarks: 'Initial scrutiny completed - all documents verified',
          status: 'APPROVED'
        }
      });

      // Step 7: Test GUARD_SCRUTINY_COMPLETE
      const scrutinyContext = {
        applicationId,
        userId: 'test-user-id',
        userRole: 'OWO',
      ...
      const scrutinyResult = await executeGuard('GUARD_SCRUTINY_COMPLETE', scrutinyContext);
      expect(scrutinyResult.canTransition).toBe(true);
      expect(scrutinyResult.reason).toContain('OWO scrutiny completed');

      // Step 8: Create BCA clearance
      const bcaSection = await prisma.wfSection.findFirst({
        where: { code: 'BCA' }
      });
      ...
      // Step 9: Create Housing clearance
      ...
      // Step 10: Test GUARD_CLEARANCES_COMPLETE
      ...
      // Step 11: Create accounts breakdown
      ...
      // Step 12: Test GUARD_ACCOUNTS_CALCULATED
      ...
      // Step 14: Test GUARD_PAYMENT_VERIFIED
      ...
      // Step 15: Create accounts review (using ACCOUNTS section as approver equivalent)
      ...
      // Step 16: Test GUARD_APPROVAL_COMPLETE
      ...
      const approvalResult = await executeGuard('GUARD_APPROVAL_COMPLETE', approvalContext);
      expect(approvalResult.canTransition).toBe(true);
      expect(approvalResult.reason).toContain('Approval completed');

      // Step 17: Create and finalize transfer deed
      ...
      // Step 18: Test GUARD_DEED_FINALIZED
      const deedContext = {
        applicationId,
        userId: 'test-user-id',
        userRole: 'ADMIN',
      ...
      // Step 19: Verify complete workflow state
      ...
      expect(finalApplication?.reviews).toHaveLength(2); // OWO and Approver
