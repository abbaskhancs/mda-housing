...
class DocumentService {
    async generateDocument(request) {
        try {
            ...
            switch (documentType) {
                case 'INTAKE_RECEIPT':
                    pdfBuffer = await pdfService_1.pdfService.generateIntakeReceipt(templateData);
                    break;
                case 'BCA_CLEARANCE':
                ...
                case 'DISPATCH_MEMO':
                    ...
                    break;
                case 'TRANSFER_DEED':
                    pdfBuffer = await pdfService_1.pdfService.generateTransferDeed(templateData);
                    break;
                default:
                    throw new Error(`Unsupported document type: ${documentType}`);
            }
            // Upload to file storage
            const { url: storageUrl, hash, filePath } = await fileStorageService_1.fileStorageService.uploadFile(pdfBuffer, fileName, 'application/pdf', {
            ...
            const downloadUrl = fileStorageService_1.fileStorageService.generateDownloadUrl(applicationId, expiresInHours * 3600);
            // Save to database
            ...
            logger_1.logger.info(`Document generated successfully: ${documentType} for application ${applicationId}`);
            return {
                id: document.id,
            ...
            logger_1.logger.error('Failed to generate document:', error);
            throw error;
        }
    }
    async generateAllDocuments(applicationId, templateData) {
        const documentTypes = [
            'INTAKE_RECEIPT',
            'BCA_CLEARANCE',
            'HOUSING_CLEARANCE',
        ...
        for (const documentType of documentTypes) {
            try {
                ...
                logger_1.logger.error(`Failed to generate ${documentType}:`, error);
    ...
    async getDocument(documentId) {
        try {
            ...
            if (!document.expiresAt || new Date() > document.expiresAt) {
                ...
                downloadUrl = fileStorageService_1.fileStorageService.generateDownloadUrl(documentId, expiresInHours * 3600);
                const expiresAt = new Date(Date.now() + (expiresInHours * 60 * 60 * 1000));
    ...
    async getApplicationDocuments(applicationId) {
        try {
            ...
            logger_1.logger.error('Failed to get application documents:', error);
            throw error;
        }
    }
    async verifyDownloadAccess(documentId, signature, expires) {
        try {
            return fileStorageService_1.fileStorageService.verifySignature(documentId, signature, expires);
        }
        catch (error) {
            logger_1.logger.error('Failed to verify download access:', error);
            return false;
        }
    }
    async downloadDocument(documentId) {
        try {
            ...
            const filePath = fileStorageService_1.fileStorageService.extractFilePath(document.storageUrl);
            // Get file from storage
            const buffer = await fileStorageService_1.fileStorageService.downloadFile(filePath);
            return buffer;
        }
        catch (error) {
            logger_1.logger.error('Failed to download document:', error);
            throw error;
        }
    }
    generateFileName(applicationId, documentType) {
    ...
    shouldGenerateDocument(applicationId, documentType, templateData) {
