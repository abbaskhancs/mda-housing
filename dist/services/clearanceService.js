...
exports.getClearanceById = exports.getClearancesByApplication = exports.createClearance = void 0;
...
const createClearance = async (applicationId, sectionId, statusId, remarks, userId, signedPdfUrl) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const existingClearance = application.clearances.find(c => c.sectionId === sectionId);
            let clearance;
        ...
        logger_1.logger.error('Error creating clearance:', error);
        throw error;
    }
};
...
const checkAutoProgress = async (applicationId, currentStageId, sectionCode, statusCode) => {
    try {
        ...
        if (!guardResult.canTransition) {
            logger_1.logger.warn(`Auto-progress blocked by guard ${guardName}: ${guardResult.reason}`);
            return undefined;
        }
        ...
        logger_1.logger.error('Error checking auto-progress:', error);
        return undefined;
    }
};
const getClearancesByApplication = async (applicationId) => {
...
exports.getClearancesByApplication = getClearancesByApplication;
const getClearanceById = async (clearanceId) => {
