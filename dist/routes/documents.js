...
router.post('/generate', async (req, res) => {
    try {
        ...
        const document = await documentService_1.documentService.generateDocument({
            applicationId,
            documentType,
            templateData,
            expiresInHours
...
router.post('/generate-all', async (req, res) => {
    try {
        ...
        const documents = await documentService_1.documentService.generateAllDocuments(applicationId, templateData);
        res.json({
            success: true,
            documents
        });
...
router.get('/application/:applicationId', async (req, res) => {
    try {
        ...
        const documents = await documentService_1.documentService.getApplicationDocuments(applicationId);
        res.json({
            success: true,
            documents
        });
...
router.get('/:documentId/download', async (req, res) => {
    try {
        ...
        // Verify signature
        const isValid = await documentService_1.documentService.verifyDownloadAccess(documentId, signature, parseInt(expires));
        if (!isValid) {
            return res.status(403).json({ error: 'Invalid or expired download link' });
        }
        // Get document buffer
        const documentBuffer = await documentService_1.documentService.downloadDocument(documentId);
        const document = await documentService_1.documentService.getDocument(documentId);
        if (!document) {
            return res.status(404).json({ error: 'Document not found' });
        }
...
router.get('/health', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Document service health check failed:', error);
